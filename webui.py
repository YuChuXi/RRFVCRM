...
with gr.Blocks(title=title_0) as demo:
    ...
    with gr.Tab("Chat and generate speech, facial movements and actions"):
        with gr.Row():
            with gr.Column():
                ...
                count_penalty = gr.Slider(0.0, 2.0, label="Count Penalty", step=0.1, value=1)
            with gr.Column():
                with gr.Row():
    ...
    with gr.Tab("chat bot"):
        ...
        with gr.Column():
            ...
            #chat_msg = chat_input.submit(chat, [chatbot, chat_input], [chatbot, chat_input])
...
demo.launch(server_name="192.168.0.105", server_port=11450, show_error=True, share=False)
